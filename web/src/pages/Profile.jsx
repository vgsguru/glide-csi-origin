...
/**
 * The arbitrator's inputs, edited by the person they belong to.
 *
 * Everything here is loaded from and written back to the backend. Saving
 * re-runs the agent immediately, because the whole point is that changing your
 * priorities visibly changes what it recommends.
 */
export default function Profile({ onChanged }) {
  ...
  return (
    ...
    >
      <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[var(--border)] pb-6 mb-6">
        <div>
        ...
        {profile?.user && (
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="font-medium">{profile.user.name || 'User'}</div>
              <div className="text-xs text-[var(--muted-foreground)]">{profile.user.email}</div>
            </div>
            <button
              onClick={() => signOut()}
              className="rounded-full bg-[var(--secondary)] px-4 py-2 text-sm font-medium hover:bg-[var(--accent)] transition-colors"
            >
              Sign out
            </button>
          </div>
      ...
      <motion.div variants={item} className="glass-strong rounded-3xl p-6 md:p-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl font-bold">Chat engine</h2>
            <p className="text-xs text-[var(--muted-foreground)]">
              Reported by the backend, not configured here.
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  setBaseUrl(baseUrl);
                  setToast({ message: 'Backend URL updated', tone: 'success' });
